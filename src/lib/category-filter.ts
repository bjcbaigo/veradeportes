...
const NINOS = ["nino", "ninos", "ni�o", "ni�os", "infantil", "kids"];
...
export function matchesCategory(
  ...
  if (cat === "Ofertas")
    return (
      (badge || "").toLowerCase().includes("oferta") || c.includes("oferta") || n.includes("oferta")
    );
  ...
  if (cat === "Ninos") return NINOS.some((w) => haystack.includes(w));
  return true;
}
